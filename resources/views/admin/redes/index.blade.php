            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header cabecalho">
                        ...
                        <div>
                            @can('user-create')
                                <a href="{{route('redes.create')}}" class="btn btn-sm btn-success"><i class="fa fa-plus">
                                        Adicionar</i></a>
                            @endcan
                        </div>
                    </div>
                        ...
                        <table class="table table-success table-striped table-hover">
                            ...
                            @foreach($rows as $row)
                                <tr>
                                    <th>{{ $n++ }}</th>
                                    <td>
                                        <i class="{{ $row->icone }} fa-2x"></i>
                                    </td>
                                    <td class="align-content-center">{{ $row->nome }}</td>
                                    <td class="align-content-center">{{ $row->url }}</td>
                                    <td>
                                        <a class="{{ (!$row->status) ? 'text-danger' : 'text-success'}}" href="{{ route('redes.status',[$row->status,$row->id]) }}">
                                            @if($row->status)
                                    ...
                                    <td class="action">
                                        @can('rede-edit')
                                            <a href="{{ route('redes.edit',$row->id) }}"
                                        ...
                                        @can('rede-delete')
                                            <form class="float-right" id="form-delete{{$row->id}}"
                                                  action="{{ route('redes.destroy', $row->id) }}"
                                                  method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button"
